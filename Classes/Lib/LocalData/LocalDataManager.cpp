...
namespace WkCocos
{
	namespace LocalData
	{
		...
		bool LocalDataManager::saveData(const std::string& saveName, std::string data, short version)
		{
			auto newentity = entity_manager->create();
			//new File component for each request. The aggregator system will detect duplicates and group them
		...
		bool LocalDataManager::loadData(const std::string& saveName, std::function<void(std::string data)> load_cb, short version)
		{
			auto newentity = entity_manager->create();
			//new File component for each request. The aggregator system will detect duplicates and group them
			newentity.assign<Comp::File>(cocos2d::FileUtils::getInstance()->getWritablePath() + saveName);
